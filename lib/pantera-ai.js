...
function buildFallbackReply(authorName, errorMessage = "") {
  if (errorMessage) {
    return `Oi ${authorName}, estou por aqui. Recebi sua mencao ao @pantera, mas encontrei um problema temporario na integracao da IA (${errorMessage}). Se quiser, tente de novo em instantes.`;
  }
...
export async function generatePanteraReply({
  ...
  const input = [
    ...
    `Origem da mencao: ${mentionSource.type}`,
  ...
  try {
    ...
    return buildFallbackReply(authorName, error.message);
  }
}
